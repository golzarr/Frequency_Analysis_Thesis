package cryptanalytic.tool.fa;

public class AndrejThesis {
...
//		String source = "Rbo rpktigo vcrb bwucja wj kloj hcjd, km sktpqo, cq rbwr loklgo vcgg cjqcqr kj skhcja wgkja wjd rpycja rk ltr rbcjaq cj cr. Roppy Lpwrsborr"
//				.toUpperCase();
		String source = "Pm ol ohk hufaopun jvumpkluaphs av zhf, ol dyval pa pu jpwoly, aoha pz, if zv johunpun aol vykly vm aol slaalyz vm aol hswohila, aoha uva h dvyk jvbsk il thkl vba."
				.toUpperCase();
	...
	private static void smartShift(int shift, int[] unicode, int[] unicodeCopy) {
		...
		if (eTotal / total >= 0.05 || aTotal / total >= 0.05 || iTotal / total >= 0.05 || oTotal / total >= 0.05
				|| uTotal / total >= 0.05) {
			System.out.println();
